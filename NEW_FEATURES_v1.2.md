...
添加了撤销（Undo）和恢复（Redo）功能，方便用户在计算过程中回退或前进到之前的状态。系统会自动保存每次计算后的状态，最多保存50个历史记录。

### 使用方法

#### GUI版本
- **撤销**：点击"撤销 (Ctrl+Z)"按钮或按 `Ctrl+Z` 快捷键
- **恢复**：点击"恢复 (Ctrl+Y)"按钮或按 `Ctrl+Y` 快捷键
- 按钮会根据是否有可撤销/恢复的操作自动启用或禁用
- 状态栏会显示操作结果
...
- 每次调用 `process_text()` 后，系统自动保存当前状态
- 历史记录包括：输入行、计算结果、变量值
- 撤销时恢复到上一个状态
- 恢复时前进到下一个状态
- 如果在历史中间位置进行新计算，后面的历史会被清除
