    ...
    def parse_line(self, line):
        ...
        if line.lower().startswith('endian:'):
            ...
            if endian in ['little', 'small', '小端', '小字节序']:
                ...
                return None, None, f"已设置为小端字节序 (Little Endian)", 'endian', False
            elif endian in ['big', 'large', '大端', '大字节序']:
                self.bit_display_mode = 'big'
                return None, None, f"已设置为大端字节序 (Big Endian)", 'endian', False
            elif endian in ['none', 'off', '关闭']:
                self.bit_display_mode = None
                return None, None, f"已关闭字节序显示", 'endian', False
            else:
                return None, None, f"错误: 未知的字节序类型: {endian}", 'endian', False
